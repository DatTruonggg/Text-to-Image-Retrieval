...
def group_result_by_video(lst_scores, list_ids, list_image_paths, KeyframesMapper):
  ...
  for i, image_path in enumerate(list_image_paths):
    data_part, video_id, frame_id = image_path.replace('/Data/KeyFrame/', '').replace('.jpg', '').split('/')
...
def search_by_filter(prev_result, text_query, k, mode, model_type, range_filter, ignore_index, keep_index, Sceneid2info, DictImagePath, CosineFaiss, KeyframesMapper):
  ...
  if model_type == 'both':
    ...
    scores_clipv2_l14, list_clipv2__l14_ids, _, _ = CosineFaiss.text_search(text_query, index=filter_idx, k=k, model_type='clipv2_l14')
    scores_clipv2_h14, list_clipv2_h14_ids, _, _ = CosineFaiss.text_search(text_query, index=filter_idx, k=k, model_type='clipv2_h14')

    lst_scores, list_ids = merge_searching_results_by_addition([scores_clip, scores_clipv2_l14, scores_clipv2_h14],
                                                               [list_clip_ids, list_clipv2__l14_ids, list_clipv2_h14_ids])
